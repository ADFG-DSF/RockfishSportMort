...
sc_wt_rm <- read.csv("data/raw_dat/Species_comp_SC/rf_mean_wt_SC.csv") %>% clean_names() %>%
  mutate(assemblage = ifelse(sp == 142,"black",
                             ifelse(sp == 145,"yelloweye","other")),
         wt_lbs = mean_wt,
         wt_cv = sd_wt / wt_lbs) %>%
  rename(area = cfmu) %>%
  right_join(lut %>% filter(region %in% c("Central","Kodiak")),by = "area")

unique(sc_wt_rm$assemblage)
unique(ly_wt$region); unique(sc_wt_rm$region)
unique(ly_wt$area)

head(sc_wt_rm); head(ly_wt)
