...
knit: (function(input_file, encoding) {
        rmarkdown::render(input_file, 
                          output_file = paste0("Annual_estimates_DEV_",Sys.Date(),".html"))
      })
---
...
#source("../scripts/bayes_data_param_load.R", echo = FALSE)
start_yr <- 1977
end_yr <- 2024
...
B4 <- "simp" #complex "simp", "none

#res <- "annual_est_take5.1.1.c_fixH.2_thru2024_50000_fullother_2026-07-09" #good start
#res <-  "annual_est_working_kodpr_thru2024_50000__2026-07-10"
#res <- "annual_est_working_kodpr.1_thru2024_50000__2026-07-10"
#res <- "annual_est_working_kodpr_thru2024_50000_bigsd_2026-07-10"
#res <- "annual_est_working_kodpr.1_thru2024_50000_dropafog_2026-07-10"
res <- "annual_est_working_kodpr.1_thru2024_50000_origprior_2026-07-10"

postH <- readRDS(paste0("..\\output\\bayes_posts\\",res,".rds"))

histres <- "Gen4int_indcomp_swhsR_FULL_pHB4pars_re0full_altwt_2xcvSEo_finaltuning7_thru2024_2e+06_SE06ex_2026-06-29"

histpost <- readRDS(paste0("..\\output\\bayes_posts\\",histres,".rds"))
...
as.data.frame(
  rbind(t(histpost$q50$H_ayg),
        t(histpost$q50$H_ayu),
        t(histpost$q50$H_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$H_ayg),
          t(histpost$q2.5$H_ayu),
          t(histpost$q2.5$H_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$H_ayg),
          t(histpost$q97.5$H_ayu),
          t(histpost$q97.5$H_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_hi95") %>%
  ...
  mutate(area = toupper(as.character(area))) %>% #filter(!is.na(area)) %>%
#  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(tot_how, by = c("year","user","area")) %>% 
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) -> histrf_plotdat
...
rf_plotdat %>% mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  ...
  geom_line(data = histrf_plotdat %>% filter(user == "All"), col = "blue", linetype = 3) +
...
as.data.frame(
  rbind(t(histpost$q50$R_ayg),
        t(histpost$q50$R_ayu),
        t(histpost$q50$R_ay))) %>%
  setNames(nm = unique(R_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$R_ayg),
          t(histpost$q2.5$R_ayu),
          t(histpost$q2.5$R_ay))) %>%
      setNames(nm = unique(R_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$R_ayg),
          t(histpost$q97.5$R_ayu),
          t(histpost$q97.5$R_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_hi95") %>%
  ...
  mutate(area = toupper(as.character(area))) %>% #filter(!is.na(area)) %>%
#  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(tot_howR, by = c("year","user","area")) %>% 
  mutate(area = factor(area, toupper(unique(R_ayg$area)), ordered = TRUE)) -> histrf_plotdat2
...
rf_plotdat2 %>% mutate(area = factor(area, toupper(unique(R_ayg$area)), ordered = TRUE)) %>%
  ...
  geom_line(data = histrf_plotdat2 %>% filter(user == "All") , col = "blue", linetype = 3) + 
  facet_wrap(. ~ area, scales = "free") +
...
as.data.frame(
  rbind(t(histpost$q50$Hb_ayg),
        t(histpost$q50$Hb_ayu),
        t(histpost$q50$Hb_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "H") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Hb_ayg),
          t(histpost$q2.5$Hb_ayu),
          t(histpost$q2.5$Hb_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Hb_ayg),
          t(histpost$q97.5$Hb_ayu),
          t(histpost$q97.5$Hb_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Hb_ayg),
          t(histpost$sd$Hb_ayu),
          t(histpost$sd$Hb_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  filter(user != "All")-> histbrf_plotdat
...
brf_plotdat %>% mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>% filter(user != "All") %>%
  ...
  geom_line() + 
  geom_line(data = histbrf_plotdat %>% filter(user == "unguided"), linetype = 2, color = "red") +
  geom_ribbon(data = histbrf_plotdat %>% filter(user == "unguided"),
              aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, color = "grey") +
#  geom_point(data = histbrf_plotdat, shape = 3) +
  geom_ribbon(aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.15, color = NA) +
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
...
as.data.frame(
  rbind(t(histpost$q50$Rb_ayg),
        t(histpost$q50$Rb_ayu),
        t(histpost$q50$Rb_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "R") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Rb_ayg),
          t(histpost$q2.5$Rb_ayu),
          t(histpost$q2.5$Rb_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Rb_ayg),
          t(histpost$q97.5$Rb_ayu),
          t(histpost$q97.5$Rb_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Rb_ayg),
          t(histpost$sd$Rb_ayu),
          t(histpost$sd$Rb_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
...
brf_rel %>%  filter(user != "All") %>%
  ...
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(x = year, y = R_how, color = user, shape = user)) +
  geom_ribbon(data = histbrf_rel %>% filter(user == "unguided"),
              aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, fill = "grey", color = NA) +
  geom_ribbon(aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histbrf_rel %>% filter(user == "unguided"), col = "blue") +
  geom_line() +
...
as.data.frame(
  rbind(t(histpost$q50$Hy_ayg),
        t(histpost$q50$Hy_ayu),
        t(histpost$q50$Hy_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "H") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Hy_ayg),
          t(histpost$q2.5$Hy_ayu),
          t(histpost$q2.5$Hy_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Hy_ayg),
          t(histpost$q97.5$Hy_ayu),
          t(histpost$q97.5$Hy_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Hy_ayg),
          t(histpost$sd$Hy_ayu),
          t(histpost$sd$Hy_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
...
YE_harv %>% filter(user != "All") %>%
  ...
  scale_color_manual(values = pal) +
  facet_wrap(. ~ area, scales = "free") +
  geom_ribbon(data = histYE_harv %>% filter(user == "unguided"),
              aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "lightblue") +
  geom_ribbon(aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_point(aes(x = year, y = H_how, color = user, shape = user)) +
  geom_line(data = histYE_harv %>% filter(user == "unguided"), color = "blue", linetype = 2) +
  geom_line() + 
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
...
as.data.frame(
  rbind(t(histpost$q50$Ry_ayg),
        t(histpost$q50$Ry_ayu),
        t(histpost$q50$Ry_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "R") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Ry_ayg),
          t(histpost$q2.5$Ry_ayu),
          t(histpost$q2.5$Ry_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Ry_ayg),
          t(histpost$q97.5$Ry_ayu),
          t(histpost$q97.5$Ry_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Ry_ayg),
          t(histpost$sd$Ry_ayu),
          t(histpost$sd$Ry_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
...
as.data.frame(
  ...
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(x = year, y = R_how, color = user, shape = user)) +
  geom_ribbon(data = histYE_rel %>% filter(user == "unguided"),
              aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "lightblue") +
  geom_ribbon(aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histYE_rel %>% filter(user == "unguided"), col = "blue") +
  geom_line() + 
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
...
as.data.frame(
  rbind(t(histpost$q50$Hd_ayg),
        t(histpost$q50$Hd_ayu),
        t(histpost$q50$Hd_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "H") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Hd_ayg),
          t(histpost$q2.5$Hd_ayu),
          t(histpost$q2.5$Hd_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Hd_ayg),
          t(histpost$q97.5$Hd_ayu),
          t(histpost$q97.5$Hd_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Hd_ayg),
          t(histpost$sd$Hd_ayu),
          t(histpost$sd$Hd_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  left_join(dsr_how, by = c("year","user","area")) %>%
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  filter(area %in% southeast) %>% 
  filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) -> histDSR_harv
...
DSR_harv %>%  filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) %>%
  ...
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(x = year, y = H_how, color = user, shape = user)) +
  geom_ribbon(data = histDSR_harv %>% filter(user == "unguided"),
              aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "grey") +
  geom_ribbon(aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histDSR_harv %>% filter(user == "unguided"), col = "blue", linetype = 2) +
  geom_line() + 
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
...
as.data.frame(
  rbind(t(histpost$q50$Rd_ayg),
        t(histpost$q50$Rd_ayu),
        t(histpost$q50$Rd_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "R") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Rd_ayg),
          t(histpost$q2.5$Rd_ayu),
          t(histpost$q2.5$Rd_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Rd_ayg),
          t(histpost$q97.5$Rd_ayu),
          t(histpost$q97.5$Rd_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Rd_ayg),
          t(histpost$sd$Rd_ayu),
          t(histpost$sd$Rd_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  filter(area %in% southeast) %>%
  filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) -> histDSR_rel
...
DSR_rel %>% filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) %>%
  ...
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(x = year, y = R_how, color = user, shape = user)) +
  geom_ribbon(data = histDSR_rel %>% filter(user == "unguided"),
              aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "grey") +
  geom_ribbon(aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histDSR_rel %>% filter(user == "unguided"), col = "blue", linetype = 2) + 
  geom_line() +
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
as.data.frame(
  rbind(t(histpost$q50$Hs_ayg),
        t(histpost$q50$Hs_ayu),
        t(histpost$q50$Hs_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "H") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Hs_ayg),
          t(histpost$q2.5$Hs_ayu),
          t(histpost$q2.5$Hs_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Hs_ayg),
          t(histpost$q97.5$Hs_ayu),
          t(histpost$q97.5$Hs_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Hs_ayg),
          t(histpost$sd$Hs_ayu),
          t(histpost$sd$Hs_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  filter(area %in% southeast) %>%
  filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) -> histslope_harv
...
slope_harv %>% filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) %>%
  ...
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(x = year, y = H_how, color = user, shape = user)) +
  geom_ribbon(data = histslope_harv %>% filter(user == "unguided"),
              aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "grey") +
  geom_ribbon(aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histslope_harv %>% filter(user == "unguided"),
            col = "blue", linetype = 2) + 
  geom_line() +
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
as.data.frame(
  rbind(t(histpost$q50$Rs_ayg),
        t(histpost$q50$Rs_ayu),
        t(histpost$q50$Rs_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "R") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Rs_ayg),
          t(histpost$q2.5$Rs_ayu),
          t(histpost$q2.5$Rs_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Rs_ayg),
          t(histpost$q97.5$Rs_ayu),
          t(histpost$q97.5$Rs_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Rs_ayg),
          t(histpost$sd$Rs_ayu),
          t(histpost$sd$Rs_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  left_join(slope_howR, by = c("year","user","area")) %>%
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  filter(area %in% southeast) -> histslope_rel
...
slope_rel %>% filter(user != "All" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) %>%
  ...
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(x = year, y = R_how, color = user, shape = user)) +
  geom_ribbon(data = histslope_rel %>% filter(user == "unguided"),
              aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "grey") +
  geom_ribbon(aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line() + 
  geom_line(data = histslope_rel %>% filter(user == "unguided"),
            col = "blue", linetype = 2) + 
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
...
```{r, fig.width = 11.5, fig.height = 8, fig.cap = paste0("**Figure ",X,".**- *Other* rockfish (non-pelagic and non-yelloweye) harvests 1996--2023. Lines and error polygons represent model estimates and points and error bars represent Howard et al estimates.")}
#BottleRocket2 2,3 or 2,4 Rushmore1 2,3,4 Darjeeling 1,2,3,4,5 FantasticFox1 3,4,5
#library(paletteer); scale_colour_paletteer_d("nationalparkcolors::Acadia")
pal <- wes_palette(name = "AsteroidCity2", n=3) 

as.data.frame(
#  rbind(t(postH$q50$Hdnye_ayg),
#        t(postH$q50$Hdnye_ayu),
#        t(postH$q50$Hdnye_ay))) %>%
  rbind(t(postH$q50$Ho_ayg),
        t(postH$q50$Ho_ayu),
        t(postH$q50$Ho_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
...
#          t(postH$q2.5$Hdnye_ay))) %>%
  ...
  #        t(postH$q97.5$Hdnye_ayu),
  #        t(postH$q97.5$Hdnye_ay))) %>%
    rbind(t(postH$q97.5$Ho_ayg),
          t(postH$q97.5$Ho_ayu),
          t(postH$q97.5$Ho_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(area = toupper(area)) %>%
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  full_join(DSR_harv %>% select(year,user,area,H_dsr = H) %>%
  full_join(slope_harv %>% select(year,user,area,H_slope = H),
            by = c("year","area","user")) %>%
  mutate(dsr_slope_sum = H_dsr + H_slope),
  by = c("year","area","user")) -> O_harv

as.data.frame(
  rbind(t(histpost$q50$Ho_ayg),
        t(histpost$q50$Ho_ayu),
        t(histpost$q50$Ho_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "H") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Ho_ayg),
          t(histpost$q2.5$Ho_ayu),
          t(histpost$q2.5$Ho_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Ho_ayg),
          t(histpost$q97.5$Ho_ayu),
          t(histpost$q97.5$Ho_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "H_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Ho_ayg),
          t(histpost$sd$Ho_ayu),
          t(histpost$sd$Ho_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(dsr_slope_sum = H_dsr + H_slope),
  by = c("year","area","user")) %>%  
  filter(user != "All") -> histO_harv

DSR_harv %>% select(year,user,area,H_dsr = H) %>%
  full_join(slope_harv %>% select(year,user,area,H_slope = H),
            by = c("year","area","user")) %>%
  mutate(dsr_slope_sum = H_dsr + H_slope) -> O_check
  
O_harv %>%  filter(user != "All") %>%
  ggplot(aes(x = year, y = H, color = user)) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(y = dsr_slope_sum)) +
  geom_ribbon(data = histO_harv %>% filter(user == "unguided"),
              aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, 
              color = NA, fill = "grey") +
  geom_ribbon(aes(x=year,ymin = H_lo95, ymax = H_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histO_harv %>% filter(user == "unguided"),
            col = "blue", linetype = 2) + 
  geom_line() +
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
  #ylim(0,) +
  labs(y = "Other Rockfish Harvests", x = "Year") 



#ggsave("../figures/bayes_model/other_harvests.png", width = 7, height = 4) 
X <- X+1
...
O_harv %>% 
            ...
            mutate(H = round(H,0),
                   ...
                   H_hi95 = round(H_hi95),
                   sd_H = round(sd_H,3)) %>%
            select(region,area,year,user,Harvests = H, sd_H, H_lo95,H_hi95)-> O_wb
...
```{r, fig.width = 11.5, fig.height = 8, fig.cap = paste0("**Figure ",X,".**- *Other* rockfish (non-pelagic and non-yelloweye) releases 1996--2023. Lines and error polygons represent model estimates and points and error bars represent Howard et al estimates.")}
#BottleRocket2 2,3 or 2,4 Rushmore1 2,3,4 Darjeeling 1,2,3,4,5 FantasticFox1 3,4,5
#library(paletteer); scale_colour_paletteer_d("nationalparkcolors::Acadia")
pal <- wes_palette(name = "AsteroidCity2", n=3) 
...
#          t(postH$q2.5$Hdnye_ay))) %>%
  ...
  #        t(postH$q97.5$Hdnye_ayu),
  #        t(postH$q97.5$Hdnye_ay))) %>%
    rbind(t(postH$q97.5$Ro_ayg),
          t(postH$q97.5$Ro_ayu),
          t(postH$q97.5$Ro_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  mutate(area = toupper(area)) %>%
  mutate(area = factor(area, toupper(unique(H_ayg$area)), ordered = TRUE)) %>%
  full_join(DSR_rel %>% select(year,user,area,R_dsr = R) %>%
  full_join(slope_rel %>% select(year,user,area,R_slope = R),
            by = c("year","area","user")) %>%
  mutate(dsr_slope_sum = R_dsr + R_slope),
  by = c("year","area","user")) -> O_rel

as.data.frame(
  rbind(t(histpost$q50$Ro_ayg),
        t(histpost$q50$Ro_ayu),
        t(histpost$q50$Ro_ay))) %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(start_yr:end_yr, times = 3),
         user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  pivot_longer(!c(year,user), names_to = "area", values_to = "R") %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q2.5$Ro_ayg),
          t(histpost$q2.5$Ro_ayu),
          t(histpost$q2.5$Ro_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_lo95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$q97.5$Ro_ayg),
          t(histpost$q97.5$Ro_ayu),
          t(histpost$q97.5$Ro_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
      pivot_longer(!c(year,user), names_to = "area", values_to = "R_hi95") %>%
    ...
    by = c("year","user","area")) %>%
  left_join(as.data.frame(
    rbind(t(histpost$sd$Ro_ayg),
          t(histpost$sd$Ro_ayu),
          t(histpost$sd$Ro_ay))) %>%
      setNames(nm = unique(H_ayg$area)) %>%
      mutate(year = rep(start_yr:end_yr, times = 3),
             user = rep(c("guided", "unguided", "All"), each = Y)) %>%
  ...
  full_join(slope_rel %>% select(year,user,area,R_slope = R),
            by = c("year","area","user")) %>%
  mutate(dsr_slope_sum = R_dsr + R_slope),
  by = c("year","area","user")) -> histO_rel
  
O_rel %>%  filter(user != "All") %>%
  ggplot(aes(x = year, y = R, color = user)) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  facet_wrap(. ~ area, scales = "free") +
  geom_point(aes(y = dsr_slope_sum)) +
  geom_ribbon(data = histO_rel %>% filter(user == "unguided"),
              aes(x=year,ymin = R_lo95, ymax = R_hi95), alpha = 0.25, 
              color = NA, fill = "grey") +
  geom_ribbon(aes(x=year,ymin = R_lo95, ymax = R_hi95, fill = user), alpha = 0.25, color = NA) +
  geom_line(data = histO_rel %>% filter(user == "unguided"),
            col = "blue", linetype = 2) + 
  geom_line() + 
  theme_bw(base_size = 14) +
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_y_continuous(labels = comma) +
  #ylim(0,) +
  labs(y = "Other Rockfish Releases", x = "Year") 



#ggsave("../figures/bayes_model/other_releases.png", width = 7, height = 4) 
X <- X+1
...
O_rel %>% 
            ...
            mutate(R = round(R,0),
                   ...
                   R_hi95 = round(R_hi95),
                   sd_R = round(sd_R,3)) %>%
            select(region,area,year,user,Releases = R, sd_R, R_lo95,R_hi95)-> O_wb
...
pG_obs <- 
  (jags_dat$Hhat_ayg/jags_dat$Hhat_ay)[,1:Y] %>%
  t() %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(#year = unique(Hhat_ayu$year[Hhat_ayu$year <= end_yr & Hhat_ayu$year >= start_yr  ]),
         year = 1977:end_yr,
         source = "observed") %>%
  pivot_longer(-c(year, source), names_to = "area", values_to = "pG")
...
rbind(pG_mod, pG_obs%>% mutate(pG_lo95 = NA, pG_hi95 = NA)) %>%
  ...
  geom_ribbon(aes(ymin = pG_lo95, ymax = pG_hi95, borders = NA, fill = source), 
              alpha = 0.2, color = NA) +
  geom_point() +
  geom_line(data = hist_pG_mod, col = "blue", linetype = 2, size = 1) +
  geom_line() +
...
pHpel_histmod <- 
  rbind(histpost$mean$pH[,,1,1] %>% t(),
        histpost$mean$pH[,,2,1] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user,source), names_to = "area", values_to = "pH")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$pH[,,1,1] %>% t(),
                  histpost$q2.5$pH[,,2,1] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user","source")) %>%
  left_join(rbind(histpost$q97.5$pH[,,1,1] %>% t(),
                  histpost$q97.5$pH[,,2,1] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user","source")) %>%
  filter(user != "all" & source != "SWHS (all RF)")
...
if (intdat == "yes"){
  z <- 1.96
  pHpel_obs %>% rbind(int %>%
                        mutate(year = year_n + start_yr-1, #1995,
...
pHye_histmod <- 
  rbind(histpost$mean$pH[,,1,2] %>% t(),
        histpost$mean$pH[,,2,2] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user,source), names_to = "area", values_to = "pH")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$pH[,,1,2] %>% t(),
                  histpost$q2.5$pH[,,2,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user","source")) %>%
  left_join(rbind(histpost$q97.5$pH[,,1,2] %>% t(),
                  histpost$q97.5$pH[,,2,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
...
if(B4 == "simp"){
  rbind(beta5_pH <- postH$q50$beta5_pH[,2],
...
pHye_obs %>% filter(user != "all" & source != "SWHS (all RF)") %>%
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = pHye_histmod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_ribbon(data = pHye_mod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_point(aes(shape = source), fill = "lightgrey", alpha = 0.75) +
  geom_line(data = pHye_histmod, aes(color = user), linetype = 2) +
...
pHnpny_histmod <- 
  rbind(histpost$mean$pH[,,1,3] %>% t(),
        histpost$mean$pH[,,2,3] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user,source), names_to = "area", values_to = "pH")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$pH[,,1,3] %>% t(),
                  histpost$q2.5$pH[,,2,3] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user","source")) %>%
  left_join(rbind(histpost$q97.5$pH[,,1,3] %>% t(),
                  histpost$q97.5$pH[,,2,3] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
...
if(B4 == "simp"){
  rbind(beta5_pH <- postH$q50$beta5_pH[,3],
...
pHnpny_obs %>% filter(user != "all" & source != "SWHS (all RF)") %>%
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = pHnpny_histmod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_ribbon(data = pHnpny_mod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_point(aes(shape = source, alpha = source)) +
  geom_line(data = pHnpny_histmod, aes(color = user), linetype = 2) +
...
pHdsr_histmod <- 
  rbind(histpost$mean$pH[,,1,4] %>% t(),
        histpost$mean$pH[,,2,4] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user,source), names_to = "area", values_to = "pH")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$pH[,,1,4] %>% t(),
                  histpost$q2.5$pH[,,2,4] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user","source")) %>%
  left_join(rbind(histpost$q97.5$pH[,,1,4] %>% t(),
                  histpost$q97.5$pH[,,2,4] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
...
pHdsr_obs <- 
  ...
  rbind(t((jags_dat$Hlbo_ayg)
          ...
          mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE),
                 p_lo95 = NA, p_hi95 = NA)) %>%
  filter(area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")) %>%
  rbind(int %>% mutate(year = year_n + start_yr-1, #1995,
                               area = unique(H_ayg$area)[area_n],
                               user = ifelse(user_n == 0, "charter", "private"),
                               source = ifelse(source == 1, "sample", "logbook"),
...
if(B4 == "simp"){
  rbind(beta5_pH <- postH$q50$beta5_pH[,4],
...
pHdsr_obs %>% filter(user != "all" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO") &
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = pHdsr_histmod, 
              aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  ...
  geom_errorbar(aes(ymax = p_hi95, ymin = p_lo95)) +
  geom_line(data = pHdsr_histmod, 
            aes(color = user), linetype = 2) +
...
pHslope_histmod <- 
  rbind(histpost$mean$pH[,,1,5] %>% t(),
        histpost$mean$pH[,,2,5] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user,source), names_to = "area", values_to = "pH")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$pH[,,1,5] %>% t(),
                  histpost$q2.5$pH[,,2,5] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user","source")) %>%
  left_join(rbind(histpost$q97.5$pH[,,1,5] %>% t(),
                  histpost$q97.5$pH[,,2,5] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
...
if(B4 == "simp"){
  rbind(beta5_pH <- postH$q50$beta5_pH[,5],
...
pHslope_obs %>% filter(user != "all" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO") &
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = pHslope_histmod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  ...
  geom_line(data = pHslope_histmod, aes(color = user), linetype = 2) +
...
bc_histmod2 <- 
  ...
  pivot_longer(-c(year, source), names_to = "area", values_to = "bc") %>%
  mutate(data = "H") %>%
  rbind(exp(histpost$q50$logbc_R) %>%
          t() %>%
          as.data.frame() %>%
          setNames(nm = unique(H_ayg$area)) %>%
          mutate(year = unique(Hhat_ay$year),
...
bc_obs <- 
  (jags_dat$Hhat_ayg/jags_dat$Hlb_ayg)[,1:Y] %>%
  t() %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(#year = unique(Hhat_ayu$year[Hhat_ayu$year <= end_yr & Hhat_ayu$year >= start_yr]),
  ...
  pivot_longer(-c(year, source), names_to = "area", values_to = "bc") %>%
  mutate(data = "H") %>% 
  rbind((jags_dat$Rhat_ayg/jags_dat$Rlb_ayg)[,1:Y] %>%
...
rbind(bc_mod2, bc_obs) %>%
  ...
  geom_point() +
  geom_line(data = bc_histmod2 %>% filter(data == "H"), col = "blue") +
  geom_line() +
...
rbind(bc_mod2, bc_obs) %>%
  ...
  ggplot(aes(x = year, y = bc, color = source)) +
  geom_point() +
  geom_line(data = bc_histmod2 %>% filter(data == "R"), col = "blue") +
  geom_line() +
...
bc_histmod2 <- 
  ...
  pivot_longer(-c(year, source), names_to = "area", values_to = "bc") %>%
  mutate(data = "H") %>%
  rbind(exp(histpost$q50$logbc_H * array(histpost$q50$bc_R_offset,
                                      dim = c(jags_dat$A,Y))) %>%
          t() %>%
          as.data.frame() %>%
          setNames(nm = unique(H_ayg$area)) %>%
...
rbind(bc_mod2, bc_obs) %>%
  ...
  ggplot(aes(x = year, y = bc, color = source)) +
  geom_point() +
  geom_line(data = histbc_mod2, col = "blue") +
  geom_line() +
  coord_cartesian(ylim = c(0, 5)) +
  geom_hline(aes(yintercept = mu_bc), data = mu_bc_H) +
  facet_wrap(. ~ area, scale = "free") + theme_bw(base_size = 14)+
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
         legend.position = "bottom",
         plot.margin = margin(t = 20, r = 5, b = 5, l = 5)) +
...
histp_pelagic_mod <- 
  rbind(histpost$mean$p_pelagic[,,1] %>% t(),
        histpost$mean$p_pelagic[,,2] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
  pivot_longer(-c(year, user), names_to = "area", values_to = "p_pelagic")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$p_pelagic[,,1] %>% t(),
                  histpost$q2.5$p_pelagic[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q97.5$p_pelagic[,,1] %>% t(),
                  histpost$q97.5$p_pelagic[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
...
rbind(beta5_pelagic <- postH$q50$beta5_pelagic,
  ...
  right_join(expand.grid(y = seq(1,end_yr-start_yr+1,1), u = c(1, 2), 
                         area = factor(comp$area, unique(H_ayg$area), ordered = TRUE)),
             by = "area") %>%
  mutate(logit_pel = beta5 + beta4 * (u - 1),
...
p_pelagic_obs %>%
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = histp_pelagic_mod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_ribbon(data = p_pelagic_mod, aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_point(aes(shape = source)) +
  geom_line(data = histp_pelagic_mod, linetype = 2) +
  geom_line(data = p_pelagic_mod, linetype = 1) +
  #geom_hline(data = p_pelagic_trend, aes(yintercept = p_pelagic), linetype = 2) +
#  geom_line(data=pel_trend, linetype = 2) +
  coord_cartesian(ylim = c(0, 1)) +
  facet_wrap(. ~ area) + theme_bw(base_size = 14)+
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
histp_black_mod <- 
  rbind(histpost$mean$p_black[,,1] %>% t(),
        histpost$mean$p_black[,,2] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user, source), names_to = "area", values_to = "p_black")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$p_black[,,1] %>% t(),
                  histpost$q2.5$p_black[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q97.5$p_black[,,1] %>% t(),
                  histpost$q97.5$p_black[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
...
p_black_obs <-
  ...
  as.data.frame() %>%
  mutate(year = comp_year + start_yr-1,
...
p_black_mod %>%
  left_join(postH$mean$beta5_black %>% t() %>% data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              pivot_longer(cols = unique(H_ayg$area),
                           names_to = "area", values_to = "beta5") %>%
              left_join(postH$mean$beta4_black %>% t() %>% data.frame() %>%
                          setNames(nm = unique(H_ayg$area)) %>%
                          pivot_longer(cols = unique(H_ayg$area),
...
rbind(beta5_black <- postH$q50$beta5_black,
  ...
  right_join(expand.grid(y = seq(1,end_yr-start_yr+1,1), u = c(1, 2), 
                         area = factor(comp$area, unique(H_ayg$area), ordered = TRUE)),
             by = "area") %>%
  mutate(logit_b = beta5 +   beta4 * (u - 1),
...
rbind(p_black_obs %>% select(year,p_black,user,area),
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = histp_black_mod %>%
                mutate(user = factor(user, levels = c("charter", "private", 
                                        "ha p black","ha charter adj.","ha private adj."))),
  ...
  geom_line(data = histp_black_mod %>%
                mutate(user = factor(user, levels = c("charter", "private", 
                                        "ha p black","ha charter adj.","ha private adj."))),
...
histp_yellow_mod <- 
  rbind(histpost$mean$p_yellow[,,1] %>% t(),
        histpost$mean$p_yellow[,,2] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user, source), names_to = "area", values_to = "p_yellow")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$p_yellow[,,1] %>% t(),
                  histpost$q2.5$p_yellow[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q97.5$p_yellow[,,1] %>% t(),
                  histpost$q97.5$p_yellow[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q50$p_yellow[,,1] %>% t(),
                  histpost$q50$p_yellow[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
...
rbind(beta5_ye <- postH$q50$beta5_yellow,
  ...
  right_join(expand.grid(y = seq(1,end_yr-start_yr+1,1), u = c(1, 2), 
                         area = factor(comp$area, unique(H_ayg$area), ordered = TRUE)),
             by = "area") %>%
  mutate(logit_y = beta5 +   beta4 * (u - 1),
...
rbind(p_yellow_obs) %>%
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = histp_yellow_mod,
              aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.15, color = NA) +
  ...
  geom_line(data = histp_yellow_mod, linetype = 2) +
...
histp_dsr_mod <- 
  rbind(histpost$mean$p_dsr[,,1] %>% t(),
        histpost$mean$p_dsr[,,2] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user, source), names_to = "area", values_to = "p_dsr")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$p_dsr[,,1] %>% t(),
                  histpost$q2.5$p_dsr[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q97.5$p_dsr[,,1] %>% t(),
                  histpost$q97.5$p_dsr[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q50$p_dsr[,,1] %>% t(),
                  histpost$q50$p_dsr[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
...
rbind(p_dsr_obs) %>% filter(area %in% southeast) %>%
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = histp_dsr_mod %>% filter(area %in% southeast),
              aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.15, color = NA) +
  ...
  geom_point() +
  #geom_point(data = p_yellowX_obs, aes(x = year, y = p_yellow), color = "yellow", size = 0.5) +
  geom_line(data = histp_dsr_mod %>% filter(area %in% southeast), linetype = 2) +
...
histp_slope_mod <- 
  rbind(histpost$mean$p_slope[,,1] %>% t(),
        histpost$mean$p_slope[,,2] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user, source), names_to = "area", values_to = "p_slope")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(histpost$q2.5$p_slope[,,1] %>% t(),
                  histpost$q2.5$p_slope[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q97.5$p_slope[,,1] %>% t(),
                  histpost$q97.5$p_slope[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(histpost$q50$p_slope[,,1] %>% t(),
                  histpost$q50$p_slope[,,2] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
...
rbind(p_slope_obs %>% filter(area %in% southeast)) %>%
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = histp_slope_mod %>% filter(area %in% southeast),
              aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.15, color = NA) +
  ...
  geom_point() +
  #geom_point(data = p_yellowX_obs, aes(x = year, y = p_yellow), color = "yellow", size = 0.5) +
  geom_line(data = histp_slope_mod %>% filter(area %in% southeast), linetype = 2) +
...
The proportion of unconverged species composition proportional estimates that are near the bounds of 0 or 1 is `r nrow(bad_ps %>% filter(median > 0.95 | median < 0.05)) / nrow(bad_ps)`

The proportion of unconverged true parameters (defined as Rhat > 1.01) that are also in excess of an Rhat of 1.1 is `r round( nrow(bad_true_params %>% filter(Rhat > 1.1))/nrow(bad_true_params),2)` while `r round( nrow(bad_true_params %>% filter(Rhat <= 1.1))/nrow(bad_true_params),2)` are less than 1.1.
...
```{r}
flextable(bad_true_params) %>%
#flextable(true_params_rha_gt1p1) %>%
  #set_caption("Table 1. Biological reference points from the base assessment model, including estimates of maximum sustained yield (MSY), stock status, exploitation rates corresponding to MSY values.",
 #             align_with_table = TRUE, style = "Table Caption") %>%
  autofit() %>%
  flextable::font(fontname = "Times New Roman", part = "all") %>%
...
wb_save(rep_wb, file = "RF_Sport_Removals_annual_update.xlsx", overwrite = TRUE, path = NULL)
