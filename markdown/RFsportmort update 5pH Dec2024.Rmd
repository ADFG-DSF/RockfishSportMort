...
knit: (function(input_file, encoding) {
        rmarkdown::render(input_file, 
                          output_file = paste0("RFsportmort_update_", "LB_fit_3pH_hybPr_splitpH_",Sys.Date(),".html"))
      })
---
...
#mod<-"LB_fit_5pH"
mod<-"LB_fit_3pH"
res <- "HR_fitLBR_2bias_hierPcomp_3pH_hybPr_splitpH_thru2023_2500000_7kn_2025-02-18"
...
\begin{equation}
\textrm{logit}(pH_{(pH)ayuc})~=~\beta1_{(pH)ayu} + \frac{\beta2_{(pH)ayuc}}{(1 + exp(\beta3_{(pH)ayuc}*(y - \beta4_{(pH)ayuc})))} + \beta5_{(pH)ayuc}*I(u=private)+re_{(pH)ayuc}
\end{equation}
...
\text{censored} \widehat{LB}_R{(nonpel,nonye)ay}~\sim~\textrm{LogNormal}\left(\log(R_{(nonpel,nonye)ay}), 1\right)\text{T}\left(\widehat{LB}_R{(nonpel,nonye)ay}, \infty\right)
\end{equation}
...
* **Model development notes:**
  + tau parameters not always converged, but generally just some spikeness. I would argue these are acceptable. 
  + pH parameters almost there except for B1 for yelloweye in NG, PWSO, and Kodiak areas and "other" rf in PWSO and SOKO2SAP. Most of these look like straight lines so amplitude should be small, but chains drifing off into la-la land.
  + p_YE parameters pretty good or getting there, except for CSEO. Looks like B3 inflection point is bimodel with one chain on lower bound. All other SE areas OK near upper bound, so further constraints will hopefull fix this. B3 for WKMA at lower bound, but is mostly a straight line so probably OK.
  + p_Pelagic parameters close. B1 for WKMA not great but thats a low data straight line. Constraining more may help. CSEO has problems with B2 and B3, although plots of those nice. May need to loosen B3 lower end?
  + p_brf parameters a mess. Most of these should be straight lines so I think the best approach is to reparameterize the model so that only CI and NSEI have the logistic curve while the other areas are fit to straight lines. Or, maybe more simply, fix the parameters in the straight lines so that amplitude (B1) and slope (B2) are 0. 
...
```{r, fig.width = 11.5, fig.height = 5, fig.cap = "**Figure 6.**- DSR rockfish harvests 1996--2023. Lines and error polygons represent model estimates and points and error bars represent Howard et al estimates."}
...
```{r, fig.width = 11.5, fig.height = 5, fig.cap = "**Figure 7.**- DSR rockfish releases 1996--2023. Lines and error polygons represent model estimates and points and error bars represent Howard et al estimates."}
#pal <- wes_palette(name = "Darjeeling1", n=3)
pal <- wes_palette(name = "AsteroidCity1", n=3)
...
```{r, fig.width = 11.5, fig.height = 5, fig.cap = "**Figure 6.**- Slope rockfish harvests 1996--2023. Lines and error polygons represent model estimates and points and error bars represent Howard et al estimates."}
#BottleRocket2 2,3 or 2,4 Rushmore1 2,3,4 Darjeeling 1,2,3,4,5 FantasticFox1 3,4,5
#library(paletteer); scale_colour_paletteer_d("nationalparkcolors::Acadia")
...
```{r, fig.width = 11.5, fig.height = 5, fig.cap = "**Figure 7.**- Slope rockfish releases 1996--2023. Lines and error polygons represent model estimates and points and error bars represent Howard et al estimates."}
#pal <- wes_palette(name = "Darjeeling1", n=3)
pal <- wes_palette(name = "Rushmore1")[c(3,4)] 
...
``` 
<br>
```{r, fig.width = 10, fig.height = 8, fig.cap = "**Figure 13.**- Annual proportion of yelloweye rockfish catch that was harvested. Note that pre-1990 estimates are used to estimate catch in these years when catch estimates are not available."}
pal <- wes_palette(name = "FantasticFox1", n=3)[c(1,5)]
...
```{r, fig.width = 10, fig.height = 5, fig.cap = "**Figure 13.**- Annual proportion of DSR rockfish catch that was harvested. Note that pre-1990 estimates are used to estimate catch in these years when catch estimates are not available. Note that the observed logbook data is for all non-pelagic, non-yelloweye fish."}
...
```{r, fig.width = 10, fig.height = 5, fig.cap = "**Figure 13.**- Annual proportion of Slope rockfish catch that was harvested. Note that pre-1990 estimates are used to estimate catch in these years when catch estimates are not available. Note that the observed logbook data is for all non-pelagic, non-yelloweye fish."}
...
```{r, fig.width = 10, fig.height = 8, fig.cap = "**Figure 14.**- Mean SWHS bias for harvests and catches. Note that a bias < 1 indicates that the SWHS *underestimates* the true value and bias > 1 indicates the survey *overestimates* the true value."}
...
```{r, fig.width = 10, fig.height = 7, fig.cap = "**Figure 15.**- Annual estimates of SWHS bias in harvests and releases for 16 commerical fishing manamgent areas, 1996-2023. Note that a bias < 1 indicates that the SWHS *underestimates* the true value and bias > 1 indicates the survey *overestimates* the true value."}
...
```{r, fig.width = 11.5, fig.height = 5, fig.cap = "**Figure 18.**- Annual estimates of the percent of the sport harvest of non-pelagic rockfish that were DSR rockfish for 6 Southeast commerical fishing manamgent areas, 1996-2023."}
pal <- wes_palette(name = "AsteroidCity1", n=3) 
