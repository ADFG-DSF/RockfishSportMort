...
mod<-"LB_fit_3pH"
res <- "Gen3aa_indcomp_swhsR_FULL_thru2023_1e+06_2025-09-11"
...
$\textrm{logit}(pH_{ayuc})~=~(\beta0_{(pH)ayuc} + \frac{\beta1_{(pH)ayuc}}{(1 + exp(\beta2_{(pH)ayuc}*(y - \beta3_{(pH)ayuc})))}) * (1 + (I(u=private) - 1) * (\beta4_{(pH)ayuc} - 1)) + re_{(pH)ayuc}$

where the $\beta$ parameters define the intercept ($\beta0$), scaling factor ($\beta1$), slope ($\beta2$), inflection point ($\beta3$) and private angler effect ($\beta4$), $y$ is the year index, $I(u=private)$ is an index variable which is 1 when the user groups is private and 0 otherwise and $re_{(comp)ayu}$ is a random effect with a non-informative prior that estimates deviation from the fit line. The random effect term allowed estimation during the historical period when data is available, but the curve defined by the above equation determined release estimates between 1977 and 1990. Unlike the compositional trends, *pH* $\beta$ parameters were modeled hierarchically by region such that each area specific $\beta$ parameter was centered around region-specific hyperpriors of the mean and standard deviation (Tables 7 - 9). Prior development and reasoning followed the same logic as the species compositions logistic curves described above. When $\beta$ parameters were inestimable as a result of no discernable change in harvest probability over the observed time period, $\beta1$ (scaling factor) and $\beta2$ (slope) were fixed to 0 so that the long term mean value was applied. 

The regional $\beta0$ and $\beta4$ hyperpriors were readily estimated and only required vague priors on the regional means that were centered normally around 0. The hyperprior for the regional standard deviation, also modeled as precision, $\tau$ where
...
Although we do not use the guided:unguided ratio of total rockfish releases in the SWHS data to directly calculate unguided releases as in the Howard methods, the SWHS does offer information on the relative number of fish caught, retained and released for each user group. Because the proportion harvested is known for guided anglers via logbook data, the difference in the proportion harvested for each user group in the SWHS data suggests whether private anglers are more or less likely to retain a rockfish relative to guided anglers. As such the SWHS was used to develop a prior to inform the $\beta4_{(pH)ayuc}$ terms that describe the offset of the *pH* logistic curve for unguided anglers (Tables 7 - 9 pH_PRIOR). SWHS data became stratified by user groups in 2011 and thus the prior for recent years for each CFMU uses values estimated from the data. The $\beta4_{(pH)ayuc}$ term was given a prior informed by the ratio of the proportion harvested by private anglers, as estimated by the SWHS ($pH_{unguided}^{SWHS}$)  to the proportion harvested by guided anglers, as estimated by the SWHS ($pH_{guided}^{SWHS}$) such that unguided:guided SWHS pH ratio, $pH_{ung}:pH_{gui}$ is defined as

$pH_{ung}:pH_{gui}=\frac{pH_{unguided_{ay}}^{SWHS}}{pH_{guided_{ay}}^{SWHS}}$  

$pH_{user_{ay}}^{SWHS}=\frac{\widehat{SWHS}_H{ayu}}{\widehat{SWHS}_R{ayu} + \widehat{SWHS}_H{ayu}}$

$se_{pHuser} = \sqrt{se_H^2\widehat{SWHS}_R{ayu}^2 + se_R^2\widehat{SWHS}_H{ayu}^2 - 2*\widehat{SWHS}_R{ayu}*\widehat{SWHS}_H{ayu}*cov*se_{H_{ay}}*se_{R_{ay}}}$

Thus $\beta4_{(pH)ayuc}$ for 2011 through present receives a prior of

$\beta4_{(pH)ay}~\sim~\textrm{LogNormal}\left(\textrm{log}(pH_{ung}:pH_{gui}), \sigma_{pH_{ung}:pH_{gui}}^2\right)$

where 

$\sigma_{pH_{ung}:pH_{gui}} = \sqrt{\frac{se_{pHung}^2}{pH_{gui}^{SWHS^2}}+\frac{pH_{ung}^{SWHS^2}*se_{pHgui}^2}{pH_{gui}^{SWHS^4}}-(2*\frac{pH_{ung}^{SWHS}*cov*se_{pHung}*se_{pHgui}}{pH_{gui}^{SWHS^3}})}$

where *cov* refers to the covariance of $pH_{guided}^{SWHS}$ and $pH_{unguided}^{SWHS}$. For years prior to 2011, each CFMU uses a prior centered around the mean of the area specific $pH_{ung}:pH_{gui}$ ratios, excluding years beyond 2019 when management actions altered angler retention probabilities. For pre-2011 years the standard deviation for the prior was estimated from a uniform distribution between 0.1 and 5 bounded on the lower end by 0.75 times the minimum observed ratio and 1.5 times the maximum observed ratio. 
...
Similar to the compositional logistic curve, a uniform prior was applied to the inflection point $\beta3$ bracketed by the time period during which data was available and did not use a hyper prior. In some instances where recent management decisions resulted in large shifts in species proportions it was necessary to constrain the prior such that the inflection point could not occur during the last two to three years of the time series. As with $\beta1$ and $\beta2$, when there was no discernible change in the proportional data over time, $\beta3$ was inestimable and necessitated fixing the parameter so that only the $\beta0$ term was estimated to determine the long term mean of that proportion. As with the $\beta1$ and $\beta2$, regional precision hyperpriors used a constrained uniform prior on the standard deviation to allow more influence of the regional mean, stabilize the model and speed convergence. 
...
Estimating the proportion of slope rockfish in the releases required adding a fifth $\beta$ term to equation LOGITSPECIESCOMP to account for yelloweye being included in $P_{(DSR)ayu}$ of the harvest. This term offset the overall $P_{(slope)ayu}$ logit curve to differentiate $P_{(slope)ayu}$ in the harvests and releases (Table 6).    
...
$\widehat{SWHS}_H{ay}~\sim~\textrm{LogNormal}\left(\textrm{log}(H_{ay}b_H{ay}), \sigma_{SWHSHay}^2\right)$

where bias in the SWHS harvest estimates $b_H{ay}$ is modeled hierarchically across years as:

$b_H{ay}~\sim~\textrm{Normal}(\mu_H{(b)a}, \sigma_H{(b)a})$

with non-informative priors on both parameters (Table 2 PRIORS). SWHS estimates of guided angler harvest $\widehat{SWHS}_H{ay1}$ are thus related to total harvest by:
...
Note that for Southcentral and Kodiak areas $H_{(nonpel,nonye)ay1}$ is equal to the total harvest minus pelagic and yelloweye harvests. For Southeast areas $H_{(nonpel,nonye)ay1}$ is equal to the sum of the DSR and slope harvests *minus* yelloweye harvests.
...
$\widehat{SWHS}_R{ay}~\sim~\textrm{LogNormal}\left(\textrm{log}(R_{ay}b_R{ay}), \sigma_{SWHS_R{ay}}^2\right)$
...
Reported guide logbook releases $\widehat{LB}_R{ay}$ are considered a census of the true releases and is related to true releases as:
...
Unguided release estimates by species were not directly informed by data and instead reflect the balancing of several information sources that include, 1) an estimate of all rockfish released by unguided anglers, $\widehat{SWHS}_R{ay}$, 2) species specific estimates of unguided angler harvests, $H_{(comp)ayu}$, 3) species-specific retention probabilities tied to guided angler retention probabilities, $pH_{(comp)ayg}$, and 4) the $pH_{unguided}^{SWHS}$:$pH_{guided}^{SWHS}$ ratios indicating unguided angler predilection for releasing fish relative to guided anglers. The SWHS provides biased estimates of unguided releases for all rockfish while unguided harvests by species are estimated from the SWHS harvest data and port sampling programs. The retention probability is informed by the retention probabilities in the guided sector as well as the $pH_{unguided}^{SWHS}$:$pH_{guided}^{SWHS}$ ratios derived from the SWHS data. These estimates interact such that the model estimates of species-specific unguided releases must sum to the bias corrected SWHS estimate of unguided releases:

$R_{ay2} = R_{(pelagic)ay2} + R_{(yelloweye)ay2} + R_{(non-pelagic, non-yelloweye)ay2}$
...
where $ae_{au}$ is the angler effect for each area and user group modeled hierarchically around a mean of 0. Predicted $P_{(black|pelagic)ayu}^{HA}$ assumed a beta distribution such that observed $P^{HA}$

$P_{(black|pelagic)ayu}^{HA} ~\sim~ beta(\alpha_{HA},\beta_{HA})$

where

$\alpha_{HA} ~=~ (\widehat{P}_{(black|pelagic)ayu}^{HA})^2 * \frac{1 - \widehat{P}_{(black|pelagic)ayu}^{HA}}{\frac{var_{P_{HA}}-1}{\widehat{P}_{(black|pelagic)ayu}^{HA}}},$

$\beta_{HA} ~=~ (\alpha_{HA}) * \frac{1}{\widehat{P}_{(black|pelagic)ayu}^{HA} - 1},$

$var_{P_{HA}} ~=~ (\widehat{P}_{(black|pelagic)ayu}^{HA} * cvP_{(black|pelagic)ayu}^{HA})^2$

where $cvP_{(black|pelagic)ayu}^{HA}$ is the coefficient of variation for the observed hydroacoustic proportions 
...
where $varN_{black}$ and $varN_{pelagic}$ are the variance of the estimated number of black and pelagic rockfish in the hydroacoustic survey, respectively (Tscherisch et al. 2023). The covariance term was omitted as it was at or near 0.
...
The model was `r prop_conv * 100`% converged based on $\hat{R}$ values below 1.1 and `r prop_conv2 * 100`% converged based on $\hat{R}$ values below 1.01 (Gelman and Rubin 1992). The maximum $\hat{R}$ was `r round(max_rhat,2)` and `r BSAISOKO2SAP_prop * 100`% of unconverged parameters were associated with the BASI and SOKO2SAP Kodiak CFMU's that had the least amount of data to inform them, as well as very high and variable bias estimates in the SWHS data. `r p_lt_1p2 * 100`% of the unconverged parameters not associated with those two Kodiak areas were below an $\hat{R}$ of 1.2 and `r p_lt_1p15 * 100`% were below 1.15. Of the non-BSAI and SOKO2SAP unconverged parameters, `r Ru_prop * 100`% were associated with unguided release estimates. Several other parameters were slow to converge and are detailed below in the review of parameters, but in general were usually associated with proportional parameters being near the bounds of 0 or 1.  
