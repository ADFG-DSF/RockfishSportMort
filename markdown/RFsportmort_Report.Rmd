...
mod<-"LB_fit_3pH"
res <- "Gen3ab_indcomp_swhsR_FULL_thru2023_2500000__2025-09-24"
...
Thus for years 2011 onward $\beta4_{(pH)ayuc}$ is modeled with a log normal prior to ensure positive values and centered on the $pH_{ay}^{ung:gui}$ ratio such that

$\beta4_{(pH)ay}~\sim~\textrm{LogNormal}\left(\textrm{log}(pH_{ay}^{ung:gui}), \sigma_{pH_{ay}^{ung:gui}}^2\right)$
...
For years prior to 2011, each CFMU uses a prior centered around the mean of the area specific $pH_{ay}^{ung:gui}$ ratios, excluding years beyond 2019 when management actions altered angler retention probabilities, such that

$\beta4_{(pH)ay}~\sim~\textrm{LogNormal}\left(\textrm{log}(\frac{1}{n} \sum_{y=2011}^{2019} ({pH_{ay}^{ung:gui}})), \sigma_{pH_{a}^{ung:gui}}^2\right)$

where the standard deviation in those years was modeled with an area-specific uniform prior between 0.1 and 5 (Table PRIORS) and $\beta4_{(pH)ayuc}$ was bounded such that the lower bound was equal to 0.75 times the lowest observed $pH_{ay}^{ung:gui}$ ratio and the upper bound was equal to 1.5 times the maximum observed $pH_{ay}^{ung:gui}$ ratio in that area. 
