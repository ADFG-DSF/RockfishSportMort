...
Angler interview data in Southeast showed discrepancies in the number of “other” rockfish released relative to the numbers reported in charter logbooks. To accommodate apparent overdispersion in the angler interview data for the slope and DSR assemblages (which combined, comprise the “other” category recorded in logbooks) a logistic-normal random effect was used to inflate the variance (Molenberghs et al. 2010; Kassahun et al. 2012) such that the observed probability of being harvested in the interview data, $pH_{(slope/DSR)ayu}^{int}$, relates to the true underlying probability of being harvested, $pH_{(slope/DSR)ayu}$, with area-specific overdispersion such that

$\textrm{logit}(pH_{(slope/DSR)ayu}^{int})~=~\textrm{logit}(pH_{(slope/DSR)ayu})+\eta_{(slope/DSR)a}$

where $\eta_{(slope/DSR)a}$ is the area-specific random effect   

$\eta_{(slope/DSR)a}~\sim~ \textrm{Normal}(0,\sigma_{\eta(slope/DSR)a})$

and $\sigma_{\eta(slope/DSR)a}$ was modelled with a uniform prior, resulting in an adjusted binomial likelihood that accommodates the overdispersed probability as

$x_{(slope/DSR)ayu}~\sim~\textrm{Binomial}(pH_{(slope/DSR)ayu}^{int}, (x_{(slope/DSR)ayu}+r_{(slope/DSR)ayu}))$
