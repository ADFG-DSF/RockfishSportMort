...
$R_{(comp)ayu}$ estimates were less well converged relative to $H_{(comp)ayu}$ estimates with `r format(nrow(bad_Rs), big.mark = ",", scientific = FALSE)` demonstrating $\hat{R}$ values (`r round(100*nrow(bad_Rs)/nrow(Rs),2)`%) above 1.01 and `r nrow(bad_Rs_1p1)` (`r round(100*nrow(bad_Rs_1p1)/nrow(Rs),2)`%) demonstrating values above 1.1. Unconverged estimates demonstrated highly skewed posteriors where the bulk of the posterior distributions were near the lower bound. Of the `r nrow(bad_Rs_1p1)` estimates with $\hat{R}$ in excess of 1.1, `r nrow(bad_Rs_1p1 %>% filter(median <= 1))` estimates (`r round(100*nrow(bad_Rs_1p1 %>% filter(median <= 1))/nrow(bad_Rs_1p1),0)`%) had medians less than 1 and `r nrow(bad_Rs_1p1 %>% filter(median <= 10))` (`r round(100*nrow(bad_Rs_1p1 %>% filter(median <= 10))/nrow(bad_Rs_1p1),0)`%) had medians less than 10. Of the `r nrow(bad_Rs_1p1 %>% filter(median > 10))` unconverged $R_{(comp)ayu}$ estimates with medians greater than 10, `r nrow(bad_Rs_1p1 %>% filter(median > 10, year < 23))` were from pre-1999 years when logbook data became available and all `r nrow(bad_Rs_1p1 %>% filter(median > 10, year < 35))` were from before 2011 when the SWHS began tracking private and charter harvests separately. The highest unconverged release estimate had a median and mean value of `r max(bad_Rs_1p1 %>% filter(median > 10) %>% select(median))` and `r max(bad_Rs_1p1 %>% filter(median > 10) %>% select(mean))`, respectively, indicating that these questionable estimates concern relatively low numbers of released fish.
