...
my_table <- data.frame(
  Issue = c("Bias in SWHS", "Species composition of releases", "Sample size limitations", "Error propogation","Time series"),
  Howard = c("Not explicitly dealt with. Relies on logbook data and ratios of guided/unguided from SWHS data to estimate unguided releases and harvests.", 
              "Assumes that species composition of releases is equal to that of the harvest",
             "Uses sample size threshholds that when not met, values are borrowed from nearby areas",
             "Error is propogated when variance estimates are available, but there is no uncertainty generated from uncertainty in the assmptions made such as species composition of the releases or when borrowing values from one area to another.",
             "1999 - present"),
  Bayes = c("Explicitly estimates bias in SWHS harvest and release estimates based on logbook data.",
...
# Print the table
kable(my_table, caption = "Example Table") %>%
...
```{r, fig.width = 11.5, fig.height = 5, fig.cap = "**Figure 29.**- Annual estimates of the percent of the sport harvest of DSR rockfish that were yelloweye rockfish for 6 Southeast commerical fishing manamgent areas, 1996-2023."}
pal <- wes_palette(name = "AsteroidCity1", n=3) 

p_yedsr_mod <- 
  rbind(postH$mean$pDSR_YE_ayg[,] %>% t(),
        postH$mean$pDSR_YE_ayu[,] %>% t()) %>%
  as.data.frame() %>%
  setNames(nm = unique(H_ayg$area)) %>%
  mutate(year = rep(unique(Hhat_ay$year), times = 2),
         user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year))),
  ...
  pivot_longer(-c(year, user, source), names_to = "area", values_to = "p_dsr")  %>%
  mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)) %>%
  left_join(rbind(postH$q2.5$pDSR_YE_ayg[,] %>% t(),
                  postH$q2.5$pDSR_YE_ayu[,] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(postH$q97.5$pDSR_YE_ayg[,] %>% t(),
                  postH$q97.5$pDSR_YE_ayu[,] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
              ...
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area","user")) %>%
  left_join(rbind(postH$q50$pDSR_YE_ayg[,] %>% t(),
                  postH$q50$pDSR_YE_ayu[,] %>% t()) %>%
              as.data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              mutate(year = rep(unique(Hhat_ay$year), times = 2),
                     user = rep(c("charter", "private"), each = length(unique(Hhat_ay$year)))) %>%
...
#  jags_dat[grep("comp_", names(jags_dat), value = TRUE)] %>%
  ...
  mutate(year = comp_year + 1976,
         area = unique(H_ayg$area)[comp_area],
         user = ifelse(comp_user == 0, "charter", "private"),
         p_yedsr = comp_ye / (comp_dsr),
         area = factor(area, unique(H_ayg$area), ordered = TRUE))

southeast <- c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO")
central <- c("CI","NG","PWSI","PWSO","BSAI","SOKO2SAP","WKMA","afognak","eastside","northeast")
  
rbind(p_yedsr_obs) %>% filter(area %in% southeast) %>%
  ggplot(aes(x = year, y = p_yedsr, color = user)) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  geom_ribbon(data = p_yedsr_mod %>% filter(area %in% southeast),
              aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.15, color = NA) +
  geom_point() +
  #geom_point(data = p_yellowX_obs, aes(x = year, y = p_yellow), color = "yellow", size = 0.5) +
  geom_line(data = p_yedsr_mod %>% filter(area %in% southeast), linetype = 2) +
  #geom_line(data = dsr_trend %>% filter(area %in% southeast)) +
  #geom_hline(data = p_yellow_trend, aes(yintercept = p_yellow), linetype = 2) +
  scale_alpha_manual(values = c(0.2, 1)) +
  coord_cartesian(ylim = c(0, 1)) +
  facet_wrap(. ~ area) + theme_bw(base_size = 16)+
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(y = "Proportion of DSR that were yelloweye", x = "Year")
```
...
#  jags_dat[grep("comp_", names(jags_dat), value = TRUE)] %>%
  ...
  mutate(year = comp_year + 1976,
         area = unique(H_ayg$area)[comp_area],
         user = ifelse(comp_user == 0, "charter", "private"),
         p_slope = comp_slope / (comp_N - comp_pelagic - comp_yellow),
         area = factor(area, unique(H_ayg$area), ordered = TRUE))
