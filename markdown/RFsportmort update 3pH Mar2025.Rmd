...
mod<-"LB_fit_3pH"
#mod<-"LB_hyb_3pH"
res <- "rf_harvest_est_kha_rm_wt_thru2023_1e+05_7kn_2025-05-21"
...
ggsave("../figures/bayes_model/rf_harvests.png") 
```
<br><br>
...
ggsave("../figures/bayes_model/rf_releases.png") 
```
<br>
...
ggsave("../figures/bayes_model/black_harvests.png") 
```
<br>
...
ggsave("../figures/bayes_model/black_releases.png") 
...
ggsave("../figures/bayes_model/ye_harvests.png") 
```
<br><br>
...
ggsave("../figures/bayes_model/ye_releases.png") 
```
...
ggsave("../figures/bayes_model/dsr_harvests.png") 
...
ggsave("../figures/bayes_model/dsr_releases.png") 
```
<br><br>
...
ggsave("../figures/bayes_model/slope_harvests.png") 
```
<br><br>
...
ggsave("../figures/bayes_model/slope_releases.png") 
```
...
rbind(postH$mean$kap[,1] %>% t(),
        postH$mean$kap[,2] %>% t()) %>% t() %>%
  as.data.frame() %>% 
...
ha_dat %>% pivot_longer(cols = c("ha.est","ha_adj"),
  ...
  mutate(user = ifelse(ha == "ha.est","ha p black",
                       ifelse(ha == "ha_adj" & user == "charter",
                              "ha charter adj.","ha private adj."))) -> ha_dat

pal <- c(wes_palette(name = "IsleofDogs1", n=2)[1],"blue","cyan3","darkcyan",wes_palette(name = "IsleofDogs1", n=2)[2]) 
...
rbind(p_black_obs %>% select(year,p_black,user,area),
  ...
  ggplot(aes(x = year, y = p_black, color = user, fill = user)) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  #scale_color_manual(values = c("Private" = pal[1], #pal["Private"],
  #                              "Charter" = pal[2], #["Charter"],
  #                              "p black adj." = c("blue"),
  #                              "p black" = "darkcyan")) +
  geom_ribbon(data = p_black_mod , aes(ymin = p_lo95, ymax = p_hi95, fill = user), 
              alpha = 0.2, color = NA) +
  geom_point() +
  #geom_point(data = ha_dat,aes(x = year, y = ha_adj, col = "p black adj.", shape = user)) +
  #geom_point(data = ha_dat,aes(x = year, y = ha.est, col = user), shape = 10) +
  #geom_point(data = ha_dat, aes(x = year, y = ha_adj, color = source), shape = 1) +
  #geom_point(data = ha_dat, aes(x = year, y = ha.est, color = source), shape = 10) +
  geom_line(data = p_black_mod, linetype = 2, aes(color = user, fill = user)) +
  geom_line(data=black_trend,aes(color = user, fill = user)) +
  scale_alpha_manual(values = c(0.2, 1)) +
  coord_cartesian(ylim = c(0, 1)) +
  facet_wrap(. ~ area) + theme_bw(base_size = 16)+
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(y = "Proportion of pelagic rockfish that were black", x = "Year") +
  #     color = "Hydroacoustic data",
  #  fill = "User") +
  #guides(
...
ggsave("../figures/bayes_model/p_dsr.png") 
```
...
ggsave("../figures/bayes_model/p_slope_harv.png") 
```
<br><br>
...
ggsave("../figures/bayes_model/p_slope_rel.png") 
```
<br><br>
