...
my_table <- data.frame(
  Issue = c("Time series","Bias in SWHS", "Species composition of releases", "Sample size limitations", "Error propagation"),
  Howard = c("1999 - present","Not explicitly accounted for. Relies on logbook data and ratios of guided/unguided from SWHS data to estimate unguided releases and harvests.", 
              "Assumes that species composition of releases is equal to that of the harvest, which is not evident in the logbook and podata.",
             "Uses sample size thresholds such that when areas fall below those thresholds values are borrowed from nearby areas or user groups.",
             "Error is propagated when variance estimates are available, but there is uncertainty associated with borrowing values from nearby areas, or the assumption of species compositions being identical in harvest and releases."),
  Bayes = c( "1977 - present","Explicitly estimates bias in SWHS harvest and release estimates based on logbook data.",
              "Recognizes different release probabilities by species / species assemblage and estimates it from logbook, portside interview, and bias corrected SWHS data",
            "Uses a hierarchical modelling approach that shares information between areas in the same region, reduces assumptions, and maximizes the use of all available data, even with small sample sizes. ",
            "Uncertainty in release estimates is more reflective of the fishery because these methods do not assume species composition is the same between harvests and releases. The hierarchical approach more accurately captures uncertainty within and between areas within a region.")
)
