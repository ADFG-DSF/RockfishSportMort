...
knit: (function(input_file, encoding) {
        rmarkdown::render(input_file, 
                          output_file = paste0("RFsportmort_update_", "LB_fit_",Sys.Date(),".html"))
      })
---
...
#res <- "HR_hybLBR_2bias_thru2023_3e+06_7kn_2024-12-16"
#res <- "HR_censLBR_thru2023_3e+06_7kn_2024-12-16"
...
The true number of rockfish released by user group, $R_{ayu}$, was assumed to be a function of the total number of fish caught times the proportion retained, or harvested, by user group $pH_{ayu}$. Thus, converting $H_{ayu}$ to total catches by user group, $C_{ayu}$, with $pH_{ayu}$ results in estimates of total releases such that

\begin{equation}
R_{ayu}~=~ C_{ayu} - H_{ayu} ~=~ \frac{H_{ayu}}{pH_{ayu}} - H_{ayu}
\end{equation} 

with total releases calculated from the sum of the user group releases. 

The proportion harvest parameters $pH_{ayu}$ were modeled using a logistic curve that would allow hindcasting based on trends in the data without extrapolating beyond the range of observed values such that
...
Annual black rockfish harvest $H_{(black)ayu}$ and catch $R_{(black)ayu}$ for each area and user group is:

\begin{equation}
H_{(black)ayu}~=~H_{ayu}P_{(pelagic)ayu}P_{(black|pelagic)ayu}
\end{equation}

where $P_{(pelagic)ayu}$ is the fraction of the annual harvest and releases for each area and user group that was pelagic rockfish and $P_{(black|pelagic)ayu}$ is the fraction of the annual harvest and releases of pelagic rockfish for each area and user group that was black rockfish.  Annual yelloweye rockfish harvest $H_{(yelloweye)ayu}$ and $C_{(yelloweye)ayu}$ for each area and user group:

\begin{equation}
H_{(yelloweye)ayu}~=~H_{ayu}(1-P_{(pelagic)ayu})P_{(yelloweye|non-pelagic)ayu}
\end{equation}

where $P_{(yellow|non-pelagic)ayu}$ is the fraction of the annual harvest and releases of non-pelagic rockfish for each area and user group that was yelloweye rockfish. Similar to how $pH_{ayu}$ was modeled, composition parameters ($P_{(pelagic)ayu}$, $P_{(black|pelagic)ayu}$, $P_{(yelloweye|non-pelagic)ayu}$) were modeled using a logistic curve that would allow hindcasting withouth extrapolating beyond the limit of observed values such that:
...
The $LB_{cens}$ and $LB_{fit}$ models have been run to almost complete convergence while the $LB_{hyb}$ model still comes up short, likely as it struggles to resolve the disparate information from the SWHS and logbook data. In particular, the *p_harvest* logistic curve struggles in this model given the retention closures during recent years in southeast. The curve of the data is more parabolic in southeast with a gradual increase in similar to other regions but a drop in recent years when retention of yelloweye was prohibited. 

The $LB_{cens}$ model is the best behaved and seems to fit the data the best, but results in much higher release estimates than the Howard methods as a result of not fitting to logbook release data and responding to the SWHS estimates. The $LB_{fit}$ model appears to most closely approximate the Howard release estimates and the model behaves well with the exception the *p_yellow_x* logit curve for Southeast which continues to resolve to a positive slope that does not appear to fit the data and is opposite of the negative slopes in the other two models. *I am not clear on why this is occurring, even with an informed prior that favors a negative slope.* The next step is to enforce a negative slope on that parameter, although I am somewhat uncomfortable with that.

It may be worth considering something other than a logistic curve but I am unclear as to the best option. During early model development I fit a number of polynomials but their use for hindcasting was dubious as hindcast extrapolation before the observations seemed spurious. This is why I decided to force a logistic curve on the data that would come in flat at the beginning of the observed period. 
...
if (mod %in% c("LB_fit","LB_hyb_2b")) { #2 bias
...
rbind(p_yellow_obs) %>%
  ...
  geom_line(data = yellow_x_trend %>% filter(area %in% southeast)) +
