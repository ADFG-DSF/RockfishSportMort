...
res <- "Gen4int_indcomp_swhsR_FULL_pHB4pars_re0full_altwt_2xcvSEo_thru2024_5e+05_noSEoR_2025-12-07"
...
pHpel_obs %>% filter(user != "all" & source != "SWHS (all RF)") %>%
  ...
  scale_alpha_manual(values = c('Logbook' = 1, 'Portside interviews' = 0.5))
...
pHdsr_obs %>% filter(user != "all" & area %in% c("CSEO","EWYKT","NSEI","NSEO","SSEI","SSEO") &
  ...
  scale_alpha_manual(values = c('Logbook (DSR & Slope)' = 0.1, 'Portside interviews' = 0.5))
}
Fig_N <- Fig_N+1
``` 
<div style="text-align:left; width: 70%; margin: auto;"> </div>
...
p_pelagic_obs %>%
  ...
  scale_alpha_manual(values = c('logbook' = 1, 'sample' = 0.5))
...
rbind(p_black_obs %>% select(year,p_black,user,area),
  ...
  geom_line(data = p_black_mod %>%
                mutate(user = factor(user, levels = c("charter", "private", 
                                        "ha p black","ha charter adj.","ha private adj."))),
            linetype = 1, aes(color = user),size = 1.1) +
  ...
  scale_alpha_manual(values = c(0.75,0.75,1,0.5,0.5))
