...
pel_harv %>% 
            ...
            mutate(H = round(H,0),
                   ...
                   H_hi95 = round(H_hi95),
                   sd_H = round(sd_H,3)) %>%
            select(region,area,year,user,Harvests = H, sd_H, H_lo95,H_hi95) -> pelH_wb
...
as.data.frame(
  ...
  left_join(as.data.frame(t(jags_dat$cvHhat_ay)) %>% setNames(nm = unique(H_ayg$area)) %>%
              ...
              pivot_longer(!year, names_to = "area", values_to = "cv") %>%
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area")) -> swhs_Hres_c

rbind(swhs_Hres_h %>% filter(year < 2020),
      swhs_Hres_c %>% filter(year >= 2020)) -> swhs_Hres
...
postH <- postH_c
  ...
  left_join(as.data.frame(t(jags_dat$cvRhat_ay)) %>% setNames(nm = unique(H_ayg$area)) %>%
              ...
              pivot_longer(!year, names_to = "area", values_to = "cv") %>%
              mutate(area = factor(area, unique(H_ayg$area), ordered = TRUE)),
            by = c("year","area"))-> swhs_Rres_c
  
swhs_Rres <- rbind(swhs_Rres_h %>% filter(year < 2020),
                   swhs_Rres_c %>% filter(year >= 2020))
...
rbind(lbRes_h %>% filter(year < 2020),
      lbRes_c %>% filter(year >= 2020)) %>%
...
mu_bc_H_h <- data.frame(area = unique(H_ayg$area), mu_bc = apply(exp(postH_h$sims.list$mu_bc_H), 2, mean))
mu_bc_H_c <- data.frame(area = unique(H_ayg$area), mu_bc = apply(exp(postH_c$sims.list$mu_bc_H), 2, mean))

mu_bc_R_h <- data.frame(area = unique(H_ayg$area), mu_bc = apply(exp(postH_h$sims.list$mu_bc_R), 2, mean))
mu_bc_R_c <- data.frame(area = unique(H_ayg$area), mu_bc = apply(exp(postH_c$sims.list$mu_bc_R), 2, mean))
...
bc_mod <- rbind(bc_mod_h %>% filter(year < 2020),
                bc_mod_c %>% filter(year >= 2020))

bc_mod2 <- rbind(bc_mod2_h %>% filter(year < 2020),
                bc_mod2_c %>% filter(year >= 2020))
...
rbind(bc_mod2, bc_obs) %>%
  ...
  geom_hline(aes(yintercept = mu_bc), data = mu_bc_H_h) +
...
rbind(bc_mod2, bc_obs) %>%
  ...
  geom_hline(aes(yintercept = mu_bc), data = mu_bc_R_h) +
  facet_wrap(. ~ area, scale = "free") + theme_bw(base_size = baseTXT)+
  theme (axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
         legend.position = "bottom",
         plot.margin = margin(t = 20, r = 5, b = 5, l = 5),
...
bias_h %>% data.frame() %>% filter(bc < 50) %>%
...
pHpel_mod <- rbind(pHpel_mod_h %>% filter(year < 2020),
                   pHpel_mod_c %>% filter(year >= 2020))
...
pHye_mod <- rbind(pHye_mod_h %>% filter(year < 2020),
                  pHye_mod_c %>% filter(year >= 2020))
...
pHnpny_mod <- rbind(pHnpny_mod_h %>% filter(year < 2020),
                    pHnpny_mod_c %>% filter(year >= 2020))
...
pHdsr_mod <- rbind(pHdsr_mod_h %>% filter(year < 2020),
                   pHdsr_mod_c %>% filter(year >= 2020))
...
pHslope_mod <- rbind(pHslope_mod_h %>% filter(year < 2020),
                     pHslope_mod_c %>% filter(year >= 2020))
...
p_pelagic_mod <- rbind(p_pelagic_mod_h %>% filter(year < 2020),
                       p_pelagic_mod_c %>% filter(year > 2020))
...
p_black_mod <- rbind(p_black_mod_h %>% filter(year < 2020),
                     p_black_mod_c %>% filter(year >= 2020))
...
p_black_mod %>%
  left_join(postH_h$mean$beta0_black %>% t() %>% data.frame() %>%
              setNames(nm = unique(H_ayg$area)) %>%
              pivot_longer(cols = unique(H_ayg$area),
                           names_to = "area", values_to = "beta0") %>%
              left_join(postH_h$mean$beta1_black %>% t() %>% data.frame() %>%
                          setNames(nm = unique(H_ayg$area)) %>%
                          pivot_longer(cols = unique(H_ayg$area),
                                       names_to = "area", values_to = "beta1"), 
                        by = "area") %>%
              left_join(postH_h$mean$beta2_black %>% t() %>% data.frame() %>%
                          setNames(nm = unique(H_ayg$area)) %>%
                          pivot_longer(cols = unique(H_ayg$area),
...
rbind(beta0_black <- postH_h$q50$beta0_black,
      beta1_black <- postH_h$q50$beta1_black,
      beta2_black <- postH_h$q50$beta2_black,
      beta3_black <- postH_h$q50$beta3_black,
      beta4_black <- postH_h$q50$beta4_black) %>% t() %>%
  data.frame() %>%
  mutate(area = unique(H_ayg$area)) %>%
  rename(beta0 = X1, beta1=X2, beta2=X3, beta3=X4, beta4 = X5) %>%
  right_join(expand.grid(y = seq(1,end_yr-start_yr+1,1), u = c(1, 2), 
...
p_yellow_mod <- rbind(p_yellow_mod_h %>% filter(year < 2020),
                      p_yellow_mod_c %>% filter(year >= 2020))
...
p_dsr_mod <- rbind(p_dsr_mod_h %>% filter(year < 2020),
                   p_dsr_mod_c %>% filter(year >= 2020))
...
p_slope_mod <- rbind(p_slope_mod_h %>% filter(year < 2020),
                     p_slope_mod_c %>% filter(year >= 2020))
...
if (intdat == "yes"){} else {
  ...
  pr_slope_mod <- rbind(pr_slope_mod_h %>% filter(year < 2020),
                        pr_slope_mod_c %>% filter(year >= 2020))
...
pG_mod <- rbind(pG_mod_h %>% filter(year < 2020),
                pG_mod_c %>% filter(year >= 2020))
...
```{r, fig.width = 10, fig.height = 10, fig.cap = paste0("**Figure ",Fig_N,".**- Total removals of black rockfish by guided and unguided anglers in 16 Alaska commercial fishery management units as dervided from harvests and release mortalities 1977-2024. The grey polygon in the backround represents 95% credibility intervals of the total removals. Southeast Alaska (CSEO, EWYKT, NSEI, NSEO, SSEI, SSEO) biomass estimates prior to ",SE4stageyear1," do not use mean weights generated from the 4-stage sampling design (Richards et al. 2025) that has been used since ", SE4stageyear1, " and as such are considered preliminary. Finalized biomass estimates for pre-",SE4stageyear1, " estimates are not expected to change significantly when 4-stage estimates are generated for those years.")}
...
```{r, fig.width = 8, fig.height = 6, fig.cap = paste0("**Figure ",Fig_N,".**- Total removals of non-black pelagic rockfish by guided and unguided anglers in Southeast Alaska commercial fishery management units as dervided from harvests and release mortalities 1977-2024. The grey polygon in the backround represents 95% credibility intervals of the total removals. Biomass estimates prior to ",SE4stageyear1," do not use mean weights generated from the 4-stage sampling design (Richards et al. 2025) that has been used since ", SE4stageyear1, " and as such are considered preliminary. Finalized biomass estimates for pre-",SE4stageyear1, " estimates are not expected to change significantly when 4-stage estimates are generated for those years.")}
